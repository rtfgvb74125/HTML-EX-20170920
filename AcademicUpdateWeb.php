        ...
        <div class="form-box">
            ...
            if($result = $connect->query($query)) {
                while ($row = $result->fetch_row()) {
                    ?>
                    <form action="AcademicUpdate.php" method = "post">
                        <?php
                        echo '<input type="hidden" name="id" class="form-control input-md" value="'.$row[0].'">'
                        ?>
                        <?php
                        echo'標題 : '. '<input  name="title" type="text"class="form-control input-md" value="' . $row[1] . '">'
                        ?>
                        <?php
                        echo '時間'.'<input  name="date" type="text"class="form-control input-md" value="' . $row[2] . '">'
                        ?>
